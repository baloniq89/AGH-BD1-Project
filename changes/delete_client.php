...
if(isset($_GET['s1']))
{
pg_query( "DELETE FROM restauracja.klienci  WHERE klient_id=$_GET[del]") or die('<h4>Błąd </h4>');
}
 $query = "SELECT * FROM restauracja.klienci";
 $result = pg_query( $query) or die('Query failed: ' . pg_last_error());

echo '<form action="delete_client.php" method="GET">';
echo "<table class='content-table'>";
 echo "<thead>";
  ...
  echo "<th> Telefon </th>";
  echo "<th> Email </th>";
 	echo "<th><input class='contact-form-btn' type=submit name=s1 value='Usuń klienta'></th>";	 
	  echo "</tr>";
	   echo "</thead>";
	    echo "<tbody>";
